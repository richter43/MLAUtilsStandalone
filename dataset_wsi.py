...
class DatasetWSI:
    def __init__(self,
        ...
        self.deepZoomObjects = [(dz.DeepZoomGenerator(slide,
                                                      ...
                                                      limit_bounds=True), label) for slide, label in self.openSlideObjects]
        ...
        self.tilePlaceholders = [self._create_tile_placeholders(slide,
                                                                label,
                                                                slide.level_count - 2,
                                                                preview) for slide, label, preview in self.deepZoomObjects]
        ...
        print("{} images of size {}x{}".format(len(self.tilePlaceholders), tile_size, tile_size))
    ...
    def _fixup_shape(self, image, label):
        ...
        """
        image.set_shape([self.input_shape[0], self.input_shape[1], self.input_shape[2]])
    ...
    def make_dataset(self):
        ...
        dataset = dataset.map(lambda x: tf.py_function(self._to_image, [x], Tout=[tf.float32, tf.float32]), num_parallel_calls=8)
